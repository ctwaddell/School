...
int main(int argc, char* argv[]) {
  ...
  if (argc == 2)
    {
      if (argv[1][0] == '-')
	{
	  write(STDERR_FILENO, "wsh: invalid command line\n", strlen("wsh: invalid command line\n"));
	  exit(1);
	}
  ...
  write(STDERR_FILENO, "Usage: wsh [batch-file]\n", strlen("Usage: wsh [batch-file]\n"));
  exit(1);
 ...
 interactive:
  ...
  skip:
	...
	int asdf = execvp(command[0], argv2);
	if (asdf == -1)
	  {
