...
void* serve_forever_wrapper(void* datArg)
    {
        int* serverfd = (int*) datArg;
        serve_forever(serverfd);
        return NULL;
    }
...
int main(int argc, char **argv) {
    ...
    PriorityQueue listenerQueue;
    initializeQueue(&listenerQueue, max_queue_size);
    free(listenerQueue.nodes);

    //num_listener = 100; //change num listeners manually if you want

    pthread_t listenerThreadID[num_listener];
    for(int i = 0; i < num_listener; i++)
    {
        pthread_create(&listenerThreadID[i], NULL, serve_forever_wrapper, &server_fd);
    }

    for (int i = 0; i < num_listener; i++)
    {
        pthread_join(listenerThreadID[i], NULL);
    }
