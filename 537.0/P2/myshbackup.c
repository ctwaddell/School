...
int main(int argc, char* argv[])
{
  ...
  if(argc == 1) // Interactive mode
    ...
    {
      if(argv[1][0] == '-') // Invalid argument/commandline
	{
	  write(STDERR_FILENO, "mysh: invalid command line\n", strlen("mysh: invalid command line\n"));
	  exit(1);
	}
      if((file = fopen(argv[1], "r")) == NULL) // File failure
	{
  ...
  else // Invalid argument/commandline
    {
 ...
 interactive:
  {
    ...
    if(strncmp(input, exitMsg, strlen(input)) == 0 || check == NULL)
      {
	goto end;
      }
	...
	if(command[redirection + 1][0] == '>' || command[redirection + 2][0] != 0 || command[redirection + 1][0] == 0)
	  {
	    write(STDERR_FILENO, "Redirection misformatted.\n", strlen("Redirection misformatted.\n"));
	    freeblocks(argv2);
	    goto interactive;
	...
	if(redirection)
	  {
		...
		write(STDERR_FILENO, command[redirection + 1], strlen(command[redirection + 1]));
		write(STDERR_FILENO, ".\n", 2);
		_exit(1);
	      }
	    close(1);
	...
	if(asdf == -1)
	  {
	    write(1, "Error: specified command does not exist (remember to include absolute path).\n", 78);
	    freeblocks(argv2);;
	    _exit(1);
	  }
      }
 ...
 batch:
  {
    ...
    while(command[i][0] != 0 && command[i][0] != '>' && i < 100)
      {
	argv2[i] = (char*)malloc(strlen(command[i]) + 1);
	strncpy(argv2[i], command[i], strlen(command[i]) + 1);
	i++;
      }
