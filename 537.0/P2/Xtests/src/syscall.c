...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    ...
    if(curproc->tf->eax == -1)
    {
      curproc->counter2 = curproc->counter2 + 1;
    }
    //cprintf("\%s, %d\n", curproc->name, curproc->tf->eax);
    if(num != SYS_fork && num != SYS_exec && num != SYS_sbrk && num != SYS_getnumsyscalls && num != SYS_getnumsyscallsgood)
    {
      curproc->counter1 = curproc->counter1 + 1;
    }
  } else {
