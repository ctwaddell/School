...
void Split(char csv[10][1024], int num_movies, char titles[10][1024], int years[10], char directors[10][1024], float ratings[10], long long dollars[10])
{
  ...
  for(int j = 0; j < num_movies; j++) //traverse each one dimensional array and figure it out
    {
      Clean_Whitespace(csv[j]);
      Fix_Case(csv[j]);
      arrayLength = Array_Count(csv[j]);
      for(int k = 0; k < arrayLength; k++) //within each one dimensional array, split up the info
	{
	  tempIndex = 0;
